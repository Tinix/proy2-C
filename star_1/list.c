...
data_t list_search(list_t list, index_t index) {
    ...
    while (list != NULL) {
        if (index_is_less_than(pair_fst(list->elem), index)) {
            list = list->next;
        } else if (index_is_equal(pair_fst(list->elem), index)) {
...
list_t list_add(list_t list, index_t index, data_t data) {
    ...
    if (list == NULL) {
        ...
        if (!index_is_less_than(pair_fst(list->elem), index)) {
            ...
            aux = list;
            /* Quiero que aux apunte al último nodo menor que index. */
            while (aux->next != NULL &&
                   index_is_less_than(pair_fst(aux->next->elem), index)) {
                aux = aux->next;
            }
            /* aux->next es NULL o mayor que index. */
            node->next = aux->next;
            aux->next = node;
        }
    }
...
list_t list_remove(list_t list, index_t index) {
    ...
    node = list;
    while (node != NULL && index_is_less_than(pair_fst(node->elem), index)) {
        prev = node;
        node = node->next;
    }
    if (node != NULL && index_is_equal(pair_fst(node->elem), index)) {
        if (prev == NULL) {
            list = node->next;
        } else {
            prev->next = node->next;
